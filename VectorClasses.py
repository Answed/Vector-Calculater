...
class Vector2:
    ...
    def __add__(self,
                other):  # Allows that two Vector2s can be added to each other. Resulting Vector is a new Vector2 to ensure that no data gets corupted and easy working with different Vectors multiple times.
    ...
    def normalize(self):  # Calculates the normalized version of a Vector2
        lengthOfVector = self.length()
        return Vector2(self.x / lengthOfVector, self.y / lengthOfVector)

    # Calculations where another vector is required for 
    def scalar_product(self, other):  # Allows to calculate the scalar from two vector2's
...
class Vector3(Vector2):
    def __init__(self, xValue, yValue, zValue, tag) -> None:
        ...
        # Function overrides which are required for vector math operations
    ...
    def normalize(self):  # Calculates the normalized version of a Vector3
        lengthOfVector = self.length()
        return Vector3(self.x / lengthOfVector, self.y / lengthOfVector, self.z / lengthOfVector, self.tag)

    # Calculations where another vector is required for 
    def scalar_product(self, other):  # Allows to calculate the scalar from two vector3's
...
class Straight2D:
    ...
    def point_is_on_straight(self, point: Vector2):
        ...
        if x_row_multiplikator == y_row_multiplikator:
            return True
        else:
            return False
    ...
    def distance_between_straights(self, other):
        helparea = self.direction.scalar_product(
            other.base_point)  # Calculates the normal vector of the helparea which is needed for the calculation of the point from the straight
        number_without_parameter = self.direction.scalar_product(
            self.base_point)  # Calculation is divided in to parts to make the finding of the parameter a lot easier
        ...
        distance_vector = self.point_on_straight(
            parameter) - other.base_point  # Calculates the new point and subtracts the other point from it to find the new position vektors
...
class Straight3D(Straight2D):
    ...
    def point_is_on_straight(self, point: Vector3):
        ...
        if x_row_multiplikator == y_row_multiplikator == z_row_multiplikator:
            return True
        else:
            return False
...
class Area:
    ...
    def coordinate_form(
            self):  # Is used to calculate the value the equasion has to have in oder that the end result is right
        return self.normal_vector.scalar_product(self.base_point)

    def print_coordinate_form(self):
        check_value = self.coordinate_form()
        print("{}x + {}z + {}y = {}".format(self.normal_vector.x, self.normal_vector.y, self.normal_vector.z,
                                            check_value))

    def distance_to_point(self, point):  # Based on the Hesse normal form
        check_value = self.coordinate_form()
        return abs((self.normal_vector.scalar_product(
            point) - check_value) / self.normal_vector.length())  # Always returns the absolut Value so always returns the positiv value.
