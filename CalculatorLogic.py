...
def generate_area(area: Area, length: int):
    ...
    y = []
    z = []
    span_striaght1 = Straight3D(area.base_point, area.span_vector1, "3D")
    span_striaght2 = Straight3D(area.base_point, area.span_vector2, "3D")
    for i in range(length):
...
def calculate_angel(vectors):
    ...
    if (DM.varify_Vectors(vectors)):
        CM.Result("The angle between the vector {} and {}\n is {}".format(vectors[0], vectors[1],
                                                                          vectors[0].angle_between_vectors(vectors[1])))
...
def calculate_distance(vectors):
    ...
    if (DM.varify_Vectors(vectors)):
        CM.Result("The distance between the straight {} and {}\n is {}".format(vectors[0], vectors[1],
                                                                               vectors[0].distance_between_straights(
                                                                                   vectors[1])))
...
def check_area(vectors):
    if (vectors[0].point_is_on_area(vectors[1])):
        ...
        CM.Result("The Point {} is not on the Area {}".format(vectors[1], vectors[2]))
...
def check_straight(vectors):
    if (vectors[0].point_is_on_straight(vectors[1])):
        ...
        CM.Result("The Point {} is not on the Straight {}".format(vectors[1], vectors[2]))


def check_point(input):
    vectors = DM.FindVectorsInInput(input)
    if vectors[0] is Area:
        check_area(vectors)
    elif vectors[0] is Straight2D or vectors[0] is Straight3D:
